...
export const buildTransactionOffline = async (
  account: string,
  accountPrvKey: string,
  chainId: string,
  refBlockNum: number,
  refBlockPrefix: number,
  now?: moment.Moment,
): Promise<any> => {
  const privateKeys = [accountPrvKey]
  const accountPublicKey = PrivateKey.fromString(accountPrvKey).toPublic().toString()
  ...
  const abiProvider = {
    getRawAbi: (accountName: string): Promise<BinaryAbi> => {
      // This base64 encoded eosio abi with system contract installed (required for buyrambytes) - https://github.com/EOS-Mainnet/governance/tree/master/eosio.system
      const base64Abi =
  ...
  const transactResult = await api.transact(transaction, { broadcast: false, sign: true })
  const result = {
    signatures: transactResult.signatures,
    serializedTransaction: Buffer.from(transactResult.serializedTransaction).toString('hex')
  }
  console.log('Generated offline transaction', result)
  return result
}
